...
return (
    ...
    {images.map((image, index) => (
        <TouchableOpacity key={index} style={styles.imageContainer} onPress={()=>navigation.navigate('FourOptions',{lesson:image.name})}>
        
        <LinearGradient
              colors={['rgba(0,0,0,0)','rgba(0,0,0,0.7)']}
              style={styles.overlay}
              onPress={()=>navigation.navigate('FourOptions',{lesson:image.name})}>
        <View>
...
const Subjects = ({navigation}) => {
  return (
    <View style={styles.container}>
      <HeaderComponent navigation={navigation} subject='English'/>
